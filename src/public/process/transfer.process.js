$(document).ready(function () {
    ...
    const addressSM = '0xB743Dd4d298798c90DC66853b7465774e4F2d1C2';
    ...
    $('#submit').click(async function () {
        connectMM().then((data) => {
            var wallet = data[0];
            var value = $("#popWallet")[0].innerText;
            contract_MM.methods.tranferAdress(value).send({ from: wallet, value: web3.utils.toWei($("#txtCoin").val(), "ether") })
...
function showData(name, phone, wallet) {
    document.getElementById('popName').innerHTML = name;
    document.getElementById('popPhone').innerHTML = phone;
    document.getElementById('popWallet').innerHTML = wallet;
    $("#popup").addClass("target");
}

function addElement(name, phone, wallet) {
    $('#table').append(`<tr>
    <td>${name}</td>
    <td>${phone}</td>
    <td>
    ...
    <td>
    <button onclick='showData("${name}","${phone}","${wallet}")' class="button">Xem thông tin</button>
    </td>
</tr>`);
}